...
open class CheckableIndicatorButton @JvmOverloads constructor(
    ...
    init {
        ...
        icon.imageTintList =
            ContextCompat.getColorStateList(context, R.color.checkable_icon_tint_selector)
        indicator.imageTintList =
            ContextCompat.getColorStateList(context, R.color.checkable_icon_tint_selector)
        label.setTextColor(
            ContextCompat.getColorStateList(
                context,
                R.color.checkable_text_color_selector
            )
        )
    }
    ...
    override fun setChecked(checked: Boolean) {
        if (this.checked != checked) {
            ...
            children
                .filter { it is Checkable }
                .forEach { (it as Checkable).isChecked = checked }
